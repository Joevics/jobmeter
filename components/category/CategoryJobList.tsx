...
export default function CategoryJobList({ initialJobs = [], pageSlug }: CategoryJobListProps) {
  const [jobs] = useState<JobUI[]>(() => initialJobs.map(transformJobToUI));

  // Increment view count client-side so it only fires on genuine user visits,
  // not during server render or cache population.
  useEffect(() => {
    if (!pageSlug) return;
    fetch(`${SUPABASE_URL}/rest/v1/rpc/increment_category_page_views`, {
      method: 'POST',
      headers: {
